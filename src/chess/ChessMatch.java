...
public class ChessMatch { // é nessa classe que teremos as regras de xadrez
	...
	// retorna uma matriz de peças de xadrez corresponde a essa partida.
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		Piece p = board.removePiece(pos);	//remove a peça que estava na posição e guarda na variável p
		piecesOnTheBoard.remove(p);	//exclui essa peça da lista de peças no tabuleiro
		
		ChessPiece newPiece = newPiece(type, promoted.getColor());	//instancia a nova peça
		board.placePiece(newPiece, pos);	//coloca a nova peça na posição da peça promovida
		piecesOnTheBoard.add(newPiece);		//add a nova peça na lista de peças no tabuleiro
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece p = (ChessPiece)board.removePiece(source); // remove a peça que está na origem
		p.increaseMoveCount();
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		p.decreaseMoveCount();
		board.placePiece(p, source); // coloca a peça na posição inicial novamente

		if (capturedPiece != null) {
			board.placePiece(capturedPiece, target); // coloca a peça capturada de volta na posição dela
			capturedPieces.remove(capturedPiece); // remove da lista de capturadas
			piecesOnTheBoard.add(capturedPiece); // coloca na lista de peças no tabuleiro
		}
	...
	// filtrando a lista de peças para achar o rei usando expressões lambda e stream
	...
	private boolean testCheck(Color color) {
		Position kingPosition = king(color).getChessPosition().toPosition(); // pega a posição do rei em formato de
																				// matriz
		List<Piece> opponentPieces = piecesOnTheBoard.stream()
				.filter(x -> ((ChessPiece) x).getColor() == opponent(color)).collect(Collectors.toList()); // cria uma lista de peças do oponente
	...
	// Operação de colocar peça passando as coordenadas do xadrez, recebe as
	// coordenadas do xadrez e depois a peça
	private void placeNewPiece(char column, int row, ChessPiece piece) {
		board.placePiece(piece, new ChessPosition(column, row).toPosition()); // coloca a peça no tabuleiro
		piecesOnTheBoard.add(piece); // coloca a peça na lista de peças no tabuleiro
	}
